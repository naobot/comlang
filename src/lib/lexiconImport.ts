 ...
 * two-column form carries neither meaning nor word class, but under a `pos_meaning` key
 * convention a key like `n_book` names both — see `deriveFromKey`. Which prefixes mean
 * what is the project's own convention, passed in as `keyPos`; a project that declares
 * none gets no guessing. This only fires for columns the file left out entirely; a blank
 * cell in a file that *has* the column is the author's choice and stays blank.
 */
...
/** Entry-key prefix → word class, as the project declares it. */
export type KeyPos = Readonly<Record<string, string>>;

/**
 * A meaning and word class guessed from an entry key, for rows whose file carried neither.
 *
 * Under a `pos_meaning` key convention — `n_book`, `a_black`, `v_become` — the part before
 * the first underscore is a part of speech and the rest is the gloss with underscores
 * standing in for spaces. Only the prefixes `keyPos` maps are honoured; a key whose prefix
 * is not one of them (`top_case`, `num_3`, `dir_evid`) yields nothing rather than a
 * misleading "case" or "3", and an empty `keyPos` therefore yields nothing at all. The
 * gloss is only trusted when the prefix was recognised — otherwise the underscore split is
 * as likely to have cut a word in half. All of it is a starting point the import review
 * shows before anything is saved.
 */
export function deriveFromKey(key: string, keyPos: KeyPos): { gloss: string; word_class: string } {
  const cut = key.indexOf("_");
  if (cut <= 0) return { gloss: "", word_class: "" };
  const word_class = keyPos[key.slice(0, cut)] ?? "";
  const rest = key.slice(cut + 1);
  return { gloss: word_class && rest ? rest.replace(/_/g, " ") : "", word_class };
}

export function parseLexiconCsv(text: string, keyPos: KeyPos = {}): ParsedImport {
  ...
  body.forEach((cells, i) => {
    ...
    const derived = deriveFromKey(key, keyPos);
